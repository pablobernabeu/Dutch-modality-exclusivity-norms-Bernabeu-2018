...
# Sidebar mean for 'CD Inflected adjective', used in the Table and Properties tabs.
# WHY: the mean is taken over the selected words that have an inflected adjective
# form, which in these data means every property and no concept. Concepts hold a
# zero only as a placeholder (see the column tooltip in the table), so they are left
# out. Properties whose inflected form is absent from SUBTLEX-NL keep their zero,
# because log10(CD + 1) is truly zero for them, and the sidebar mean for
# 'Contextual diversity (CD)' treats absent words in the same way. The divisor follows the selection,
# whereas it used to be fixed at 336, which understated the mean after any filtering.
mean_inflected_CD = function(words) {
  has_inflected_form = !is.na(words$inflected_prop) & words$inflected_prop != ''
  # With only concepts selected there is nothing to average, and 'NaN' would read as a fault
  if (!any(has_inflected_form)) return('Mean = n/a (no properties)')
  paste0('Mean = ', sprintf("%.2f", round(mean(words$inflected_adj_freq_lg10CD_SUBTLEXNL[has_inflected_form]), 2)))
}
...
reactive(cat(mean_inflected_CD(selected_words())))
...
output$properties_png = downloadHandler(
  ...
  content = function(file){
    png(file, units='in', width=11, height=9.8, res=700)
...
reactive(cat(mean_inflected_CD(selected_props())))
...
HTML(
  kable(
    spread(properties_Dutch2_results[,c('Dependent', 'Variable', 'coefficients_and_asterisks')],
           Variable, coefficients_and_asterisks), format = 'html', escape = FALSE
  ) %>%
