...
observeEvent(input$properties_loadings, {
  showModal(modalDialog(
    title = HTML('<div style="padding-bottom:0px; font-size:14px; text-align:justify;"> <b>Loadings of principal components (PC) in the form of correlations.</b> Coefficients above <i>r</i> = .70 (i.e., 50% shared variance) shown in bold. When the three modalities are reduced to two principal components, the visual and haptic modalities load on the components similarly--both with positive polarities--, whereas the auditory modality is relatively independent--with a negative polarity.</div>'),
...
observeEvent(input$concepts_loadings, {
  showModal(modalDialog(
    title = HTML('<div style="padding-bottom:0px; font-size:14px; text-align:justify;"> <b>Loadings of principal components (PC) in the form of correlations.</b> Coefficients above <i>r</i> = .70 (i.e., 50% shared variance) shown in bold. When the three modalities are reduced to two principal components, visual and haptic ratings share one component, whereas the auditory modality is relatively independent, having its own component.</div>'),
    HTML( 	# Below, table constructed
      concepts_modality_loadings %>%
        # Highlight correlations above .7
        mutate(PC1 = ifelse(abs(as.numeric(PC1)) > .7, cell_spec(PC1, "html", bold = TRUE, color = 'black'), 
...
### <span style = 'font-size: 16px !important;'> Principal component analysis (PCA; see loadings on the sidebar) reflecting different relationships among the modalities (cf. <a href="#section-cf-lc-english-norms">Lynott & Connell's 2013 data for English</a>) </span> {style="padding-top: 17px !important; text-align:justify; padding-bottom: 5px !important; padding-left: 4px !important; padding-right: 4px !important;"}
...
observeEvent(input$properties_lexicals_loadings, {
  showModal(modalDialog(
    title = HTML('<div style="padding-bottom:0px; padding-left:2px; font-size:14px; text-align:justify;"> <b>Loadings of principal components for properties (see \'PC\' dependent variables), in the form of correlations.</b> Coefficients above <i>r</i> = .70 (i.e., 50% shared variance) shown in bold. </div>'),
    HTML( 	# Below, table constructed
      properties_lexicals_loadings %>%
      # Highlight correlations above .7
        mutate(PC1 = ifelse(abs(as.numeric(PC1)) > .7, cell_spec(PC1, "html", bold = TRUE, color = 'black'),
...
observeEvent(input$concepts_lexicals_loadings, {
  showModal(modalDialog(
    title = HTML('<div style="padding-bottom:0px; padding-left:2px; font-size:14px; text-align:justify;"> <b>Loadings of principal components for concepts (see \'PC\' dependent variables), in the form of correlations.</b> Coefficients above <i>r</i> = .70 (i.e., 50% shared variance) shown in bold. </div>'),
    HTML( 	# Below, table constructed
      concepts_lexicals_loadings %>%
      # Highlight correlations above .7
        mutate(PC1 = ifelse(abs(as.numeric(PC1)) > .7, cell_spec(PC1, "html", bold = TRUE, color = 'black'),
