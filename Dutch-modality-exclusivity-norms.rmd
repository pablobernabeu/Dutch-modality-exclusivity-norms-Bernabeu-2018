...
The norms, which are ratings of linguistic stimuli, served a twofold purpose: first, the creation of linguistic stimuli (see also Speed & Majid, 2017), and second, the replication of Lynott and Connell's (2009, 2013) analyses. In the collection of the ratings, forty-two respondents rated the properties and concepts separately and in blocks, with an average of eight respondents per word, a minimum of five, and a maximum of nine. The [instructions](https://osf.io/ungey/) to participants were similar to those used by Lynott and Connell (2009, 2013), except that we elicited three modalities (auditory, haptic, visual) instead of five.
...
The modality ratings collected in these norms were validated in a modality switch experiment, revealing that switches across trials with different dominant modalities (e.g., visual versus auditory) incurred semantic processing costs ([Bernabeu, Willems, & Louwerse, 2017](https://mindmodeling.org/cogsci2017/papers/0318/index.html)). All data for that study are [available](https://osf.io/97unm/wiki/home/), including a [dashboard](https://pablobernabeu.shinyapps.io/ERP-waveform-visualization_CMS-experiment/).

The [properties](#properties){style='background-color:#F2F7FF'} and the [concepts](#concepts){style='background-color:#F2F7FF'} were separately analysed. Both sets demonstrated distinct relationships among modalities, whereby the visual and haptic modalities were close together, whereas the auditory modality was relatively independent ([cf. Lynott & Connell's data for English](#cf-lc-english-norms){style='background-color:#F2F7FF'}). This ties in with findings that conceptual processing in language can collate modalities based on language statistics (Louwerse & Connell, 2011).

<div style = "padding-top: 5px; padding-left: 25px; padding-bottom: 8px; float: right; width: 22%; font-size: 85%;"> <div style = "padding-right: 35px; padding-left: 35px; text-align: center !important; padding-bottom:0.5em;"><img src="https://upload.wikimedia.org/wikipedia/commons/e/e7/Booba-Kiki.svg" alt="Bouba-Kiki" width=90% height=90%/></div> <div style="text-align: justify !important;"> When people are asked to match the words 'bouba' and 'kiki' to the above objects, most name the rounded object 'bouba' and the spiky one 'kiki'. This has been interpreted as sound symbolism, insofar as pronouncing the word 'bouba' puts the lips into a more rounded shape than 'kiki' does (Köhler, 1929; see also Cuskley, Simner, & Kirby, 2017). </div> <div style="font-size:65%; padding-top:6px;"> https://commons.wikimedia.org/wiki/File:Booba-Kiki.svg. Monochrome version 1 June 2007 by BendžVectorized with Inkscape --Qef (talk) 21:21, 23 June 2008 (UTC) [CC BY-SA 3.0 (http://creativecommons.org/licenses/by-sa/3.0/)] </div> </div>

The norms also served to investigate [sound symbolism](#sound-symbolism){style='background-color:#F2F7FF'}, which is the relation between the form of words and their meaning. The form of words rests on their sound more than on their visual or tactile properties (for oral speakers, at least). Therefore, the auditory ratings of words should more reliably predict their lexical properties (length, frequency, distinctiveness) than would haptic of visual ratings. By means of regression analyses following Lynott and Connell (2013), we found that auditory ratings were either the best predictor, or yielded a different polarity to the haptic and visual ratings, thus demonstrating sound symbolism.
  ...
  showModal(modalDialog(
    ...
    HTML( 	# Below, table constructed
      properties_modality_loadings %>%
        ...
        footnote(general = 'PC = Principal component. Correlations above r = .70 (i.e., over 50% shared variance) shown in bold.', general_title = '') %>%
...
observeEvent(input$properties_lexicals_loadings, {
  showModal(modalDialog(
    title = HTML('<div style="padding-bottom:0px; padding-left:2px; font-size:14px; text-align:justify;"> <b>Loadings of principal components</b> (see \'PC\' dependent variables) for properties. Correlations above <i>r</i> = .70 (i.e., over 50% shared variance) shown in bold. </div>'),
    HTML( 	# Below, table constructed
      properties_lexicals_loadings %>%
      # Highlight correlations above .7
        mutate(PC1 = ifelse(abs(as.numeric(PC1)) > .7, cell_spec(PC1, "html", bold = TRUE, color = 'black'),
...
**Sound symbolism is the relation between the form of words and their meaning.** The form of words rests on their sound more than on their visual or tactile properties (for oral speakers, at least). Therefore, the auditory ratings of words should more reliably predict their lexical properties (length, frequency, distinctiveness) than would haptic of visual ratings. By means of regression analyses following Lynott and Connell (2013; see [Table 6](https://link.springer.com/article/10.3758%2Fs13428-012-0267-0#Sec4)), we found that auditory ratings were either the best predictor, or yielded a different polarity to the haptic and visual ratings, thus demonstrating sound symbolism. Standardised coefficients ($\beta$) presented below, alongside significance based on t-value. Regression assumptions observed, i.e., normal distribution of residuals (transformations attempted), largest variance inflation factor < 10 and its mean around 1, tolerance > 0.2 ([Field, Miles, & Field, 2012](#info)). [External corpora used](#info). Key: 'PC' = Kaiser-normalised, varimax-rotated principal component. <sup>\*\*\*</sup>*p* < .001; <sup>\*\*</sup>*p* < .01; <sup>\*</sup>*p* < .05.
